...
class SGPR_deprecated(SGPRBase_deprecated):
    ...
    def predict_f(self, Xnew: InputData, full_cov=False, full_output_cov=False) -> MeanAndVariance:
        ...
        kuf = Kuf(self.inducing_variable, self.kernel, X_data)
        kuu = Kuu(self.inducing_variable, self.kernel, jitter=default_jitter())
        Kus = Kuf(self.inducing_variable, self.kernel, Xnew) # K_u*
...
class GPRFITC(SGPRBase_deprecated):
    ...
    def common_terms(self):
        ...
        Kdiag = self.kernel(X_data, full_cov=False)
        kuf = Kuf(self.inducing_variable, self.kernel, X_data) # Covariance matrix bw inducing points (u) and training (f)
        kuu = Kuu(self.inducing_variable, self.kernel, jitter=default_jitter()) # Covariance matrix bw inducing points (u)

        Luu = tf.linalg.cholesky(kuu)  # => Luu Luu^T = kuu
        V = tf.linalg.triangular_solve(Luu, kuf)  # => V^T V = Qff = kuf^T kuu^-1 kuf

        diagQff = tf.reduce_sum(tf.square(V), 0)
        nu = Kdiag - diagQff + self.likelihood.variance # diag(K_f - Qff) + sigma^2 I
...
class GPRPITC(GPRFITC):
    """
        GP regression with the PITC approximation.
        The key reference is
        ::
          @inproceedings{
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(data,  kernel, inducing_variable)
        self.num_inducing = int(0.01 * self.data[0].shape[0]) if num_inducing is None else num_inducing


    # REVIEW:
    def construct_blocks(self, block_sizes=None):
        # [A reasonable choice, also recommended by Tresp (1999) may be to choose
        # k = n/m blocks]
        X_data, _ = self.data
    ...
    def maximum_log_likelihood_objective(self) -> tf.Tensor:
        return self.pitc_log_marginal_likelihood()
    ...
    def pitc_log_marginal_likelihood(self):
        ...
        #  log(nu) returns inf values (not valid or reduce_sum) -> take only diagonal part
        constantTerm = -0.5 * self.num_data * tf.math.log(tf.constant(2.0 * np.pi, default_float()))
...
class GPRPIC(GPRPITC):
    """
            GP regression with the PIC approximation.
            The key reference is
            ::
                @inproceedings{DBLP:journals/jmlr/SnelsonG07,
                  author    = {Edward Lloyd Snelson and
                               Zoubin Ghahramani},
                  ...
                  biburl    = {https://dblp.org/rec/journals/jmlr/SnelsonG07.bib},
                  bibsource = {dblp computer science bibliography, https://dblp.org}
                }
            """

    def predict_f(self, Xnew: InputData, full_cov=False, full_output_cov=False) -> MeanAndVariance:
        _, _, Luu, L, alpha, beta, gamma = self.common_terms()
        Kus = Kuf(self.inducing_variable, self.kernel, Xnew)  # [M, N]
        ...
        # mean = tf.linalg.matmul(w, tmp, transpose_a=True) + self.mean_function(Xnew)  # => w -> sqrt(Q*n); tmp -> sqrt(Q*n)*[Kn+sigma^2*I]^-1*y
        ...
        test_blocks = self.km.predict(Xnew)
        sparse_blocks = self.km.predict(self.inducing_variable.variables)
...
class LocalGPR():
    def __init__(self,
        ...
        self.data = data
        self.kernel = kernel
        self.num_blocks = num_blocks if num_blocks is not None else int(self.data[0].shape[0]*0.01)
        self.create_models()

    def construct_blocks(self):
        X_data, _ = self.data
        km = KMeans(n_clusters=self.num_blocks)
        km.fit(X_data)
        self.blocks = km.labels_
        self.km = km
    ...
    def create_models(self):
        ...
        for b in range(self.num_blocks):
            X_data_b=tf.gather(X_data, np.where(self.blocks==b)[0])
            Y_data_b=tf.gather(Y_data, np.where(self.blocks==b)[0])
            data = (X_data_b, Y_data_b)
            kernel = gpflow.kernels.Matern32() if self.kernel=="Matern32" else gpflow.kernels.SquaredExponential()
            self.models.append(gpflow.models.GPR(data, kernel=kernel))

    def optimize(self, optimizer="Scipy"):
        times = []
        opt = gpflow.optimizers.Scipy() if optimizer=="Scipy" else gpflow.optimizers.NaturalGradient()
        for model in self.models:
            loss_closure = gpflow.models.util.training_loss_closure(model, model.data)
            start = timer()
            opt.minimize(
                loss_closure,
    ...
    def predict_y(self, Xnew: InputData):
        ...
        preds_means = np.zeros(Xnew.shape[0]).reshape(-1,1)
        preds_var = np.zeros(Xnew.shape[0]).reshape(-1,1)
        for b in range(self.num_blocks):
            X_test_b = tf.gather(Xnew, np.where(blocks == b)[0])
            m,c = self.models[b].predict_y(X_test_b)
            preds_means[np.where(blocks == b)[0]] = m
            preds_var[np.where(blocks == b)[0]] = c
    ...
    def predict_f(self, Xnew: InputData):
        ...
        for b in range(self.num_blocks):
            X_test_b = tf.gather(Xnew, np.where(blocks == b)[0])
            m,c = self.models[b].predict_f(X_test_b)
            preds_means[np.where(blocks == b)[0]] = m
            preds_var[np.where(blocks == b)[0]] = c
